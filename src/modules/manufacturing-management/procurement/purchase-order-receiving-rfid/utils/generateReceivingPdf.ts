...
function renderSignatures(doc: jsPDF, startY: number, receiverName: string) {
    ...
    doc.text(receiverName || "â€”", receivedByX, startY + 4);
...
export async function generateReceivingPdf(
    ...
    const doc = await PdfEngine.generateWithFrame(templateName, companyData, async (doc, startY, config) => {
        ...
        const tableBody = items.map((item) => {
            ...
            return [barcode, name, batchExpLines || "â€”", uom, String(expected), String(received), formatMoney(price), discType, formatMoney(received * discountAmount), formatMoney(net)];
        });
        ...
        doc.text(`RECEIVING RECEIPT â€” ${statusText}`, pageWidth / 2, detailsY, { align: "center" });
