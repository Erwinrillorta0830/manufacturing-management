...
// âœ… MERGED: Types for on-the-fly tagging modal
...
type Ctx = {
    // âœ… list (keep both naming styles)
    ...
    // âœ… NEW: receipt saved signal (non-breaking)
    ...
    // âœ… EXTRA: Add Product via Supplier Picker
    ...
    // âœ… CHECKLIST: Product verification (mirrors manual module)
    ...
    // âœ… LEGACY COMPAT: kept for TagRFIDStep internal usage
    markProductAsVerified: (productId: string, porId: string) => void;
    activeProductId: string | null;
    setActiveProductId: (id: string | null) => void;

    // âœ… LOTS: dropdown options
    lots: LotOption[];
    lotsLoading: boolean;
    ...
    // âœ… STEP state for workbench coordination
    step: number;
...
const API_URL = "/api/manufacturing/procurement/purchase-order-receiving-rfid";

// âœ… PERSISTENCE: localStorage helpers
const DRAFT_KEY_PREFIX = "mm_rfid_draft_";
...
export function ReceivingProductsProvider({ children, receiverId }: { children: React.ReactNode, receiverId?: number }) {
    ...
    // âœ… STEP state for workbench coordination (Step 1 is Tagging, Step 2 is Finalization)
    const [step, setStep] = React.useState(1);

    // âœ… NEW FLOW: Product Barcode Verification State
    ...
    // âœ… PERSISTENCE: Auto-save draft to localStorage whenever tagging state changes
    React.useEffect(() => {
    ...
    const openPOById = React.useCallback(
        async (poId: string, options?: { silent?: boolean }) => {
            ...
            if (!silent) {
                ...
                // âœ… avoid stale PO if server blocks or errors
                setSelectedPO(null);
            }
            ...
            try {
                ...
                if (!silent) {
                    ...
                    if (storedEditingId) {
                        ...
                        // âœ… Do NOT auto-restore reverted receipts. User must explicitly click 'Edit' in history.
                        ...
                        if (!restoredDraft) {
                            ...
                            if (hasDraftData && draft) {
                                // RFID tags are persisted as they are scanned. Restore only
                                // legacy local extra-item drafts; restoring scan counts here
                                // would count the server's tags a second time after reload.
                                ...
                                if (detail) clearDraft(detail.id);
                                setLocalScannedRfids([]);
                                setActivity([]);
                                setScannedCountByPorId({});
                                setVerifiedPorIds([]);
                                ...
                                setReceiptType("");
                                setReceiptDate(todayYMD());
                                toast.info("Saved RFID tags loaded from the server.");
                            } else {
                                setReceiptDate(todayYMD());
                                setReceiptNo("");
                                setReceiptType("");
    ...
    const removeActivity = React.useCallback(async (id: string) => {
        ...
        if (row.rfid) {
            try {
                ...
                toast.error("Failed to remove RFID tag.", { description: (error as Error).message });
                return;
            }
        }
    ...
    const scanRFID = React.useCallback(async (rfidOverride?: string) => {
        ...
        const value = (rfidOverride ?? rfid).trim().toUpperCase();
        if (!value) {
            playBeep("error");
        ...
        if (value.length !== 24 || !/^[0-9A-F]{24}$/.test(value)) {
            playBeep("error");
            const errorMsg = "Invalid RFID. Must be exactly 24 hex characters.";
        ...
        try {
            // âœ… 1. Session Duplicate Check (Client-side)
            ...
            // âœ… 2. Database Tag & Check (Server-side)
            const r = await fetch(API_URL, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            // âœ… 3. Logic based on Backend Status
            if (data.status === "unknown" || data.status === "tagged") {
                ...
                // âœ… Check Over-Receiving AFTER confirmed success
            ...
            // âœ… Already Received Check (status === "received")
    ...
    const loadReceipt = React.useCallback(async (rNo: string) => {
        ...
        try {
            ...
            // âœ… Restore Metadata from POR items
            ...
            // âœ… Restore receiptType from load_receipt API response
            ...
            if (allHaveRfid) {
                ...
                toast.success("Receipt loaded. All items have RFID tagsâ€”proceeding directly to Details & Finalization.");
            } else {
    ...
    const saveReceipt = React.useCallback(async (porMetaData?: Record<string, { lotId: string; batchNo: string; expiryDate: string }>) => {
        ...
        try {
            ...
            // âœ… gather items for printing (All items in PO, with their current statuses)
            ...
            refreshList();
            // âœ… PERSISTENCE: Clear draft on successful save
            clearDraft(String(poId));
            resetSession();

            // âœ… IMPORTANT: prepare a new receipt immediately (supports multiple receipts)
            setReceiptDate(todayYMD());
            setReceiptNo("");
            setReceiptType("");
        } catch (e: unknown) {
    ...
    const value: Ctx = {
        ...
        // âœ… LEGACY COMPAT (used by TagRFIDStep)
