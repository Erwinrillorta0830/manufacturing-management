...
import { generateRfidManifestPdf, RfidManifestRow } from "../../utils/generateRfidManifestPdf";
...
export function TagRFIDStep() {
    ...
    const manifestRows = React.useMemo<RfidManifestRow[]>(() => {
        return allItems.flatMap((item) => {
            const sessionTags = (activity || [])
                .filter((entry) => entry.porId === item.porId && entry.status === "ok")
                .map((entry) => entry.rfid);
            const tags = Array.from(new Set([...(item.rfids || []), ...sessionTags]));
            return tags.map((rfid) => ({
    ...
    const printManifest = React.useCallback(() => {
        if (!selectedPO || manifestRows.length === 0) {
            toast.error("No RFID tags to print", { description: "Scan at least one RFID tag before printing the manifest." });
            return;
        }

        generateRfidManifestPdf({
            poNumber: selectedPO.poNumber,
            supplierName: selectedPO.supplier.name,
            generatedAt: new Date().toLocaleString(),
            rows: manifestRows,
        });
        toast.success("RFID manifest generated.");
    }, [manifestRows, selectedPO]);
    ...
    // Pagination â€” filtered to current product
            ...
            <div className="space-y-4 animate-in fade-in zoom-in-95 duration-200">
                                ...
                                >
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] text-amber-700/70 dark:text-amber-400/60 font-mono mt-0.5">
                                            {h.receiptDate ? h.receiptDate.split("T")[0] : "â€”"}
                                        </span>
                                    </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            ...
                            <div className="flex flex-wrap items-center justify-end gap-2">
                                <Button
                                    ...
                                    title={manifestRows.length === 0 ? "Scan at least one RFID tag first" : "Print RFID manifest"}
                                >
                                    <Printer className="h-3.5 w-3.5" /> Print RFID Manifest
                                </Button>
                                <Button
                                    variant="outline"
                                    size="sm"
                ...
                <div className="flex flex-col items-end gap-3 pt-4">
                    <p className="text-right text-xs text-muted-foreground">
                        RFID tags are saved as they are scanned. Assign lots and complete the receipt in QA Receiving.
                    </p>
                    <Button
                        ...
                        onClick={continueToQaReceiving}
                    >
                        Continue to QA Receiving
                    </Button>
                </div>
