...
export function ReviewReceiptStep({ onBack, receiverName }: { onBack: () => void; receiverName?: string }) {
    ...
    return (
        <div className="space-y-4">
                        ...
                        <Card className="p-4 bg-muted/30 shadow-sm border">
                            ...
                            <div className="space-y-2.5 text-xs">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="font-bold text-foreground">{selectedPO?.poNumber ?? "â€”"}</span>
                                </div>

                                <div className="flex justify-between items-center">
                                    <span className="text-muted-foreground">Supplier:</span>
                                    <span className="font-bold text-foreground">{selectedPO?.supplier?.name ?? "â€”"}</span>
                                </div>
                    ...
                    <Card className="p-4 shadow-sm border">
                        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                />
                                <Button variant="ghost" size="sm" className="font-black uppercase tracking-wider text-xs" onClick={onBack}>â† Back to Tagging</Button>
                            </div>
                        </div>
                                        ...
                                        return paginatedItems.map((it: ReceivingPOItem) => {
                                            ...
                                            return (
                                                <TableRow key={porId}>
                                                    ...
                                                    <TableCell className="text-right text-xs text-destructive font-medium">{(discA || 0) > 0 ? `${formatPHP(discA * scanned)}` : "â€”"}</TableCell>
                        ...
                        {filteredItems.length > 10 && (
                            <div className="flex items-center justify-between px-4 py-3 border rounded-md bg-muted/10 mt-2">
                                <span className="text-xs text-muted-foreground font-medium font-mono">
                                    Showing {(reviewPage - 1) * 10 + 1}â€“{Math.min(reviewPage * 10, filteredItems.length)} of {filteredItems.length} items
                                </span>
            ...
            {/* âœ… Partial Receipt Confirmation Modal */}
