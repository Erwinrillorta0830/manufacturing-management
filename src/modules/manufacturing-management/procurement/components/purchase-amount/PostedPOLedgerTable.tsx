...
function AuditLoadingSkeleton() {
    return (
        <div className="space-y-4" role="status" aria-label="Loading posted ledger records">
            <span className="sr-only">Loading posted ledger records...</span>
            <div className="grid grid-cols-2 gap-3 rounded-xl border bg-muted/20 p-3 md:grid-cols-4">
                {Array.from({ length: 4 }, (_, index) => <LoadingPlaceholder key={`audit-summary-skeleton-${index}`} className="h-10 w-full" />)}
            </div>
            <div className="rounded-xl border bg-muted/20 p-4">
                <LoadingPlaceholder className="h-3 w-36" />
                <LoadingPlaceholder className="mt-2 h-4 w-52" />
            </div>
            <div className="space-y-2">
                <LoadingPlaceholder className="h-3 w-48" />
                <div className="overflow-hidden rounded-xl border bg-background p-3">
                    <div className="space-y-3">
                        {Array.from({ length: 3 }, (_, index) => <LoadingPlaceholder key={`audit-line-skeleton-${index}`} className="h-8 w-full" />)}
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                ...
                <div className="flex items-center gap-3 rounded-xl border bg-card/60 p-4 backdrop-blur-xs">
                    ...
                    <div><div className="text-[11px] font-bold uppercase text-muted-foreground">Total Foreign USD Volume</div>{loading ? <LoadingPlaceholder className="mt-1 h-6 w-32" /> : <div className="font-mono text-lg font-black">${totalForeignValue.toLocaleString("en-US", { minimumFractionDigits: PROCUREMENT_MONEY_DECIMAL_SCALE, maximumFractionDigits: PROCUREMENT_MONEY_DECIMAL_SCALE })}</div>}</div>
                </div>
            </div>
...
export function PurchaseAmountAuditView({ purchaseOrderId, postingSuccessPurchaseOrder }: PurchaseAmountAuditViewProps) {
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetchPurchaseAmountDetails(purchaseOrderId, { includePosted: true }),
            fetchPurchaseOrderArchiveStatus(purchaseOrderId).catch(() => null)
        ])
    ...
    useEffect(() => {
        if (!postingSuccessPurchaseOrder || successToastShown.current) return;
        successToastShown.current = true;
        toast.success(`Purchase amounts for ${postingSuccessPurchaseOrder} were posted successfully. Costs are now locked.`);
        router.replace(`/mm/purchase-amount/${purchaseOrderId}`, { scroll: false });
    }, [postingSuccessPurchaseOrder, purchaseOrderId, router]);
    ...
    const purchaseOrderNo = String(purchaseOrder?.purchase_order_no || purchaseOrder?.reference_number || `PO ${purchaseOrderId}`);
    const currencyCode = String(purchaseOrder?.currency_code || "PHP").toUpperCase();
    const exchangeRate = Number(poDetails?.landedCost?.computation?.exchange_rate ?? purchaseOrder?.exchange_rate);
    const totalForeign = Number(purchaseOrder?.total_foreign_currency) || 0;
    const totalPhp = Number(purchaseOrder?.total_amount ?? purchaseOrder?.total_php_value) || 0;
    const expenses = (poDetails?.landedCost?.expenses?.length ? poDetails.landedCost.expenses : poDetails?.importExpenses) || [];

    return (
        <div className="w-full space-y-5" data-testid="purchase-amount-audit-page">
                    ...
                    {loadingDetails ? (
                        ...
                        <div className="space-y-5">
                            <div className="grid grid-cols-2 gap-3 rounded-xl border bg-muted/20 p-3 text-xs md:grid-cols-4">
                                <div><div className="text-[10px] font-bold text-muted-foreground">Exchange Rate</div><div className="font-mono font-bold">{Number.isFinite(exchangeRate) && exchangeRate > 0 ? `PHP ${exchangeRate.toFixed(6)} / ${currencyCode}` : "Unavailable — reconciliation required"}</div></div>
                                <div><div className="text-[10px] font-bold text-muted-foreground">Total Foreign Currency</div><div className="font-mono font-bold">{currencyCode} {totalForeign.toFixed(PROCUREMENT_MONEY_DECIMAL_SCALE)}</div></div>
