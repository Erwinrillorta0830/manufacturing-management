...
export const ProductDetailsTab: React.FC<ProductDetailsTabProps> = ({
    ...
    const isNoBom = editedDetails.has_bom !== undefined
        ...
        <div className="w-full space-y-6">
            <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                    ...
                    {isNoBom ? (
                        <span className="bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
                            No BOM
                        </span>
                    ) : (
                        <span className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
                            Has BOM
                        </span>
                    )}
                </div>
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {/* 1. Identity & Classification Card */}
                <div className="bg-card/55 border border-border/80 rounded-xl p-5 shadow-sm space-y-4 flex flex-col justify-between">
                            ...
                            <div className="space-y-1">
                                <label className="text-[11px] font-bold text-muted-foreground uppercase flex items-center justify-between">
                                    ...
                                    {familyChildren.length > 0 && isParent && (
                                        <span className="text-[10px] font-bold text-amber-600 dark:text-amber-400 bg-amber-500/10 px-1.5 py-0.5 rounded border border-amber-500/20">
                                            Read-Only (Has {familyChildren.length} Child{familyChildren.length > 1 ? "ren" : ""})
                                        </span>
                                    )}
                                </label>
                                {familyChildren.length > 0 && isParent ? (
                                ...
                                {familyChildren.length > 0 && isParent && (
                                    <p className="text-[10px] text-muted-foreground">
                                        Serves as base parent for {familyChildren.length} registered child packaging variant{familyChildren.length > 1 ? "s" : ""}.
                                    </p>
                                )}
                            </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="space-y-1">
                                    <label className="text-[11px] font-bold text-muted-foreground uppercase">UOM Count <span className="text-red-500">*</span></label>
                                    <input 
                                        type="number" 
                                        required
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-1">
                                    <label className="text-[11px] font-bold text-muted-foreground uppercase">Density (KG/L) <span className="text-red-500">*</span></label>
                                    <input 
                                        type="number" 
                                        step="0.001"
                                        required
                                ...
                                <div className="space-y-1">
                                    <label className="text-[11px] font-bold text-muted-foreground uppercase">
                                        Maintaining Qty <span className="text-red-500">*</span>
                                    </label>
                                    <input 
                                        type="number" 
                                        step="0.01"
                                        min="0"
                                        required
                                        value={editedDetails.maintaining_quantity !== undefined && editedDetails.maintaining_quantity !== null ? editedDetails.maintaining_quantity : ""}
                                        onFocus={(e) => {
                                            if (e.target.value === "0") {
                                                handleDetailChange("maintaining_quantity", "");
                                            } else {
                                                e.target.select();
                                            }
                                        }}
                                        ...
                                        onChange={e => handleDetailChange("maintaining_quantity", e.target.value === "" ? "" : Number(e.target.value))}
                                        onBlur={(e) => {
                                            if (e.target.value.trim() === "" || isNaN(Number(e.target.value)) || Number(e.target.value) < 0) {
                                                handleDetailChange("maintaining_quantity", 0);
                                            }
                                        }}
                                        aria-invalid={!!fieldError("maintainingQuantity")}
                                        aria-describedby={fieldError("maintainingQuantity") ? "edit-maintaining-error" : undefined}
                                        className={`w-full rounded-lg border bg-background px-3 py-1.5 text-sm text-foreground outline-none focus:ring-1 focus:ring-primary transition-all ${fieldError("maintainingQuantity") ? "border-red-500 focus:ring-red-500" : "border-border"}`}
                                    />
                                    {fieldError("maintainingQuantity") && <p id="edit-maintaining-error" className="text-[10px] text-red-600" role="alert">{fieldError("maintainingQuantity")}</p>}
                                </div>
                            </div>

                            <div className={`grid gap-3 ${!isNoBom ? "grid-cols-2" : "grid-cols-1"}`}>
                                {!isNoBom && (
                                    <div className="space-y-1">
                            ...
                            <div className="space-y-1 pt-1">
                                ...
                                <textarea 
                                    ...
                                    placeholder="Enter brief product overview or packaging specifications..."
                                ...
                                {selectedProduct.identityKey && (
                                    <p className="text-[10px] text-muted-foreground">
                                        System identity: <span className="font-mono text-foreground/80">{selectedProduct.identityKey}</span>
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="space-y-3">
                            <div className="grid grid-cols-2 gap-3 items-center">
                                ...
                                <div className="space-y-1">
                                    <label className="text-[11px] font-bold text-muted-foreground uppercase">BOM Specification</label>
                                    <div className="flex items-center justify-between h-[34px] px-3 rounded-lg border border-border bg-background">
                                        <span className="text-xs font-semibold text-foreground select-none">
                                            {!isNoBom ? "Has BOM" : "No BOM"}
                                        </span>
                                        <label className="relative inline-flex items-center cursor-pointer shrink-0">
                                            <input
                                                ...
                                                checked={!isNoBom}
                                                onChange={(e) => {
                                                    handleDetailChange("has_bom", e.target.checked);
                                                }}
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    {isNoBom ? (
                                        <input 
                                            ...
                                            step="0.01"
                                            min="0"
                                            value={editedDetails.cost_per_unit !== undefined && editedDetails.cost_per_unit !== null ? editedDetails.cost_per_unit : ""}
                                            onFocus={(e) => {
                                                if (e.target.value === "0") {
                                                    handleDetailChange("cost_per_unit", "");
                                                } else {
                                                    e.target.select();
                                                }
                                            }}
                                            ...
                                            onChange={e => handleDetailChange("cost_per_unit", e.target.value === "" ? "" : Number(e.target.value))}
                                            onBlur={(e) => {
                                                if (e.target.value.trim() === "" || isNaN(Number(e.target.value)) || Number(e.target.value) < 0) {
                                                    handleDetailChange("cost_per_unit", 0);
                                                }
                                            }}
                                            className="w-full rounded-lg border border-border bg-background pl-7 pr-3 py-1.5 text-sm text-foreground outline-none focus:ring-1 focus:ring-primary transition-all font-mono font-bold"
                                            placeholder="0.00"
                                        />
                                    ) : (
                                        <input 
                                ...
                                <p className="text-[10px] text-muted-foreground">
                                    {!isNoBom 
                                        ? "Calculated from BOM ingredients, routings, labor, overheads, and yield." 
                                        : "Standard unit acquisition or inventory cost for this item."}
                                </p>
                            </div>
                            ...
                            <div className={`grid gap-3 ${!isNoBom ? "grid-cols-2" : "grid-cols-1"}`}>
                                ...
                                {!isNoBom && (
                                    <div className="space-y-1">
                                        <label className="text-[11px] font-bold text-muted-foreground uppercase">Custom Overhead (₱)</label>
                                        <input 
                                            type="number" 
                                            step="0.01" 
                            ...
                            <div className="space-y-1.5 pt-1">
                                ...
                                <div className="flex items-center gap-3 border border-dashed border-border rounded-xl p-3 bg-muted/5 hover:bg-muted/10 transition-all">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-1">
                                        <p className="text-xs font-medium text-foreground truncate">
                                            {editedDetails.product_image ? "Image attached" : "Select product image"}
                                        </p>
                                        ...
                                        {imageUploadError && (
                                            <p className="text-[10px] text-destructive truncate" role="alert">
                                                {imageUploadError}
                                            </p>
                                        )}
                                    </div>
                ...
                {/* 4. Product Family Tree Card (Span full width across all 3 columns) */}
                <div className="bg-card/55 border border-border/80 rounded-xl p-5 shadow-sm space-y-4 col-span-1 md:col-span-2 lg:col-span-3">
                    <h3 className="text-sm font-semibold text-foreground border-b pb-2 flex items-center gap-2">
