...
import { BatchCombobox } from '@/modules/manufacturing-management/shared/components/BatchCombobox';
...
export interface SalesReturnLotBatchModalProps {
  open: boolean;
...
export function SalesReturnLotBatchModal({
  open,
  onOpenChange,
  branchId,
  productId,
        ...
        (lotsData || []).forEach((lot) => {
          ...
          onhandForLot.forEach((bo) => {
            ...
            if (pId > 0 && addQty > 0) {
              ...
              if (!existing.type && (bo.productTypeId || bo.productTypeName)) {
                existing.type = bo.productTypeId || bo.productTypeName;
              }
  ...
  const handleAddLotGroup = () => {
    ...
    if (!nextLot) {
      toast.error('No additional qualified storage lots available for this product and UOM.');
      return;
    }
    ...
    const defaultBatchNo = firstBatch?.batch_no || '';
    const defaultMfg = firstBatch?.manufacturing_date || (initialValues?.manufacturing_date ? String(initialValues.manufacturing_date).substring(0, 10) : '');
    const defaultExp = firstBatch?.expiry_date || (initialValues?.expiry_date ? String(initialValues.expiry_date).substring(0, 10) : '');

    // Calculate smart remaining quantity to allocate
    const targetTotal =
      requestedQuantity ||
    ...
    setLotGroups((prev) => [
      ...prev,
      {
        ...
        allocated_quantity: unallocatedRemainder,
        active_batch_count: lotBatchCountMap.get(nextId) || 0,
        current_stock_quantity: lotStockQtyMap.get(nextId) || 0,
        batches: [
          {
  ...
  const handleRemoveLotGroup = (index: number) => {
    if (lotGroups.length <= 1) return;
    setLotGroups((prev) => prev.filter((_, i) => i !== index));
  };

  // Change the assigned storage lot for an allocation group
  const handleChangeLot = (groupIndex: number, newLotIdStr: string) => {
    const newLotId = Number(newLotIdStr);
    const selectedLot = lots.find((l) => Number(l.lot_id) === newLotId);
    if (!selectedLot) return;

    const isSelectedLotBad = isBadStockLot(selectedLot);
      ...
      prev.map((g, i) => {
        if (i === groupIndex) {
          ...
          return {
            ...
            max_batch_capacity: selectedLot.max_batch_capacity || 10,
            unit_id: selectedLot.unit_id ?? null,
            unit_name: selectedLot.unit_name ?? null,
  ...
  const handleReallocateLots = () => {
    ...
    toast.success('Allocations re-balanced according to lot capacities.');
  };

  // Update a specific field of a batch row
  const handleUpdateBatchField = (
    groupIndex: number,
    batchIndex: number,
    field: keyof BatchRowAllocation,
    value: string | number | null | QAStatus
  ) => {
    setLotGroups((prev) =>
      prev.map((g, i) => {
        if (i === groupIndex) {
  ...
  // Select a batch number and automatically resolve associated metadata
    ...
    setLotGroups((prev) =>
      prev.map((g, i) => {
        if (i === groupIndex) {
          ...
          if (!resolvedInvLotId && cleanBatch) {
            const matchedInv =
              branchInvLotsList.find(
                (ib) =>
                  String(ib.batch_no || '').trim().toLowerCase() === cleanBatch &&
                  getLotId(ib) === Number(g.lot_id)
              ) ||
              branchInvLotsList.find(
                (ib) => String(ib.batch_no || '').trim().toLowerCase() === cleanBatch
              );
            if (matchedInv?.inventory_lot_id) {
              resolvedInvLotId = Number(matchedInv.inventory_lot_id);
            }
          }
        ...
        <DialogHeader className="p-5 border-b border-border bg-card shrink-0 flex flex-row items-center justify-between">
          <div className="space-y-1">
            <DialogTitle className="text-lg font-bold text-foreground flex items-center gap-2">
              ...
              Sales Return - Lot &amp; Batch Allocation
            </DialogTitle>
          ...
          {loading ? (
            ...
            <>
              ...
              {lotGroups.map((group, gIdx) => {
                ...
                const lotTotalDeficit = groupDeficits.reduce((sum, d) => sum + d.deficit, 0);
                const absDeficit = Math.abs(lotTotalDeficit);
                const offsettingDeficitQty = Math.min(absDeficit, groupQtyTotal);
                const remainingDeficitQty = Math.max(0, absDeficit - groupQtyTotal);
                const netNewStockQty = Math.max(0, groupQtyTotal - absDeficit);
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Badge
                            ...
                            className={`text-[10px] font-mono font-bold flex items-center gap-1 ${
                              isNearCapacity ? 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/30' : ''
                            }`}
                          >
