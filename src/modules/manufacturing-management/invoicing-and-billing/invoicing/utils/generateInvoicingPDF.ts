...
const getImageDataUrl = async (originalUrl: string): Promise<string> => {
    ...
    const fetchUrl = originalUrl.startsWith("/")
        ? originalUrl
        : `/api/manufacturing/invoicing-and-billing/invoicing/proxy-image?url=${encodeURIComponent(originalUrl)}`;
...
const generateThermalReceipt = async (data: ReceiptData, existingDoc?: jsPDF): Promise<jsPDF> => {
    ...
    try {
        let logoDataUrl = '';
        try {
            logoDataUrl = await getImageDataUrl('/vertex_logo_receipt.png');
        } catch {
            logoDataUrl = await getImageDataUrl('/men2.png');
        }
