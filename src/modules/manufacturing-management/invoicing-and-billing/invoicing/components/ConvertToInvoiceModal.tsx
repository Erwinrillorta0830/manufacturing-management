...
export const ConvertToInvoiceModal: React.FC<ConvertToInvoiceModalProps> = ({
        ...
        if (isOpen && order) {
            ...
            Promise.all([
                ...
                .then((results) => {
                    ...
                    if ((convData as ConversionData)?.consolidation_error) {
                        toast.error((convData as ConversionData).consolidation_error!);
                    }
                    ...
                    if (order.void_invoices && order.void_invoices.length > 0 && invoiceDetails.length > 0) {
                        ...
                        // ── Normal order: start with empty receipt (no auto-selection/auto-distribution) ──
                        setReceipts([{ id: "1", receipt_no: "", items: [] }]);
                        setActiveReceiptId("1");
                    }
                })
    ...
    const updateItemQty = (receiptId: string, productId: number, newQty: number) => {
        ...
        // If trying to add NEW quantity but pool is already empty and order is already satisfied
        if (newQty > 0 && currentPoolRemaining <= 0 && orderNeeded <= 0) {
    ...
    // Dynamically builds formatted thermal lines for any receipt
    const buildThermalLines = (receipt: Receipt) => {
        ...
        groupKeys.forEach((key, idx) => {
            ...
            // Group label
            itemBlocks.push(thermalCenterText(`Discount: ${dtName}`));

            // Items in this group
            for (const item of groupItems) {
                const nameLines = wrapThermalText(item.product_name.toUpperCase(), THERMAL_LINE_WIDTH);
                const qtyPart = `${item.qty}${item.unit_shortcut || 'pcs'} @${formatThermalAmount(item.unit_price)}`;
        ...
        return [
            ...
            ...wrapThermalText(`Receipt#: ${receipt.receipt_no || "N/A"}`, THERMAL_LINE_WIDTH),
            ...
            ...itemBlocks,
            thermalCheckerDivider,
            thermalKeyValue("GROSS AMOUNT:", formatThermalAmount(totals.grossTotal)),
            thermalKeyValue("DISCOUNT AMOUNT:", formatThermalAmount(totals.discountTotal)),
            thermalKeyValue("NET AMOUNT:", formatThermalAmount(totals.netTotal)),
            thermalCheckerDivider,
            "",
            ...
            thermalCenterText(format(new Date(), "yyyy-MM-dd hh:mm a")),
        ];
    };
    ...
    return (
        <>
            ...
            <DialogContent className="max-w-[95vw] md:max-w-6xl p-0 overflow-hidden border-none shadow-2xl bg-background/95 backdrop-blur-xl flex flex-col h-[90vh]">
                <DialogHeader className="p-6 pb-4 bg-gradient-to-r from-primary/10 via-background to-transparent border-b flex-shrink-0">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between gap-6">
                            ...
                            {/* Center Group: Flow Logistics + KPIs */}
                            <div className="flex-1 flex items-center justify-center gap-6">
                                {/* Flow & Logistics Info */}
                                <div className="flex items-center gap-2">
                                    {(() => {
                                        if (isLoadingDetails || !conversionData) {
                                            return (
                                                <div className="flex flex-row items-center gap-2 px-3.5 py-2 bg-muted/30 dark:bg-muted/20 rounded-2xl border border-muted/40 shadow-inner">
                                                    <span className="inline-block w-2 h-2 rounded-full bg-muted-foreground/40 animate-pulse" />
                                                    <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground/70">Checking Consolidation...</span>
                                                </div>
                                            );
                                        }

                                        const latestLogistics = logistics[logistics.length - 1];
                                        const consolNo = conversionData?.consolidator_no
                                            || (conversionData?.items?.[0]?.consolidator_no && conversionData.items[0].consolidator_no !== "N/A" ? conversionData.items[0].consolidator_no : null)
                                            || (latestLogistics?.consolidation_no && latestLogistics.consolidation_no !== "N/A" && latestLogistics.consolidation_no !== "—"
                                                ? latestLogistics.consolidation_no
                                                : null);

                                        const isDirectOrder = !consolNo && (
                                            conversionData?.is_direct !== undefined
                                                ? Boolean(conversionData.is_direct)
                                                : Boolean(latestLogistics?.is_direct)
                                        );

                                        const isConsolidationError = !isDirectOrder && (!consolNo || Boolean(conversionData?.consolidation_error) || Boolean(latestLogistics?.error));

                                        return (
                                            <>
                                                {/* History Logistics (Optional) - SHOWN ONLY IF RECYCLED WITH PRIOR CONSOL */}
                                                {logistics.length > 1 && logistics[0]?.consolidation_no && logistics[0].consolidation_no !== "N/A" && logistics[0].consolidation_no !== "—" && (
                                                    <div className="flex flex-row items-center gap-2 px-3 py-1.5 bg-orange-500/5 rounded-2xl border border-orange-500/20 shadow-inner opacity-75">
                                                        <PackageSearch size={12} className="text-orange-500/70" />
                                                        <div className="flex flex-col">
                                                            <span className="text-[7px] font-black uppercase text-orange-600/60 tracking-wider leading-tight">PREV CONSOL</span>
                                                            <span className="text-[9px] font-black text-orange-700/90">{logistics[0].consolidation_no}</span>
                                                        </div>
                                                    </div>
                                                )}

                                                {/* Flow Status (Strictly no silent fallback) */}
                                                {isDirectOrder ? (
                                                    <div className="flex flex-row items-center gap-2 px-3.5 py-2 bg-blue-500/10 dark:bg-blue-500/20 rounded-2xl border border-blue-500/20 shadow-inner">
                                                        <span className="inline-block w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                                                        <span className="text-[10px] font-black uppercase tracking-wider text-blue-700 dark:text-blue-300">DIRECT ORDER</span>
                                                    </div>
                                                ) : isConsolidationError ? (
                                                    <div className="flex flex-row items-center gap-2 px-3.5 py-2 bg-destructive/10 dark:bg-destructive/20 rounded-2xl border border-destructive/30 shadow-inner" title={conversionData?.consolidation_error || "Consolidation batch record not found"}>
                                                        <span className="inline-block w-2 h-2 rounded-full bg-destructive animate-ping" />
                                                        <span className="text-[10px] font-black uppercase tracking-wider text-destructive">CONSOLIDATION ERROR</span>
                                                    </div>
                                                ) : (
                                                    <div className="flex flex-row items-center gap-2.5 px-4 py-2 bg-emerald-500/10 dark:bg-emerald-500/20 rounded-2xl border border-emerald-500/20 shadow-inner">
                                                        <PackageSearch size={14} className="text-emerald-600 dark:text-emerald-400" />
                                                        <div className="flex flex-col">
                                                            <span className="text-[8px] font-black uppercase text-emerald-600/70 dark:text-emerald-400/70 tracking-wider leading-tight">CONSOLIDATION</span>
                                                            <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-300">{consolNo}</span>
                                                        </div>
                                                    </div>
                                                )}
                                            </>
                                ...
                                <div className="hidden lg:flex flex-row items-center gap-6 px-5 py-2.5 bg-gradient-to-tr from-primary/10 to-transparent border border-primary/20 rounded-2xl shadow-inner transition-all duration-300">
                                    {(() => {
                                        ...
                                        const isSelected = !!kpiItem;

                                        const totalAllocated = kpiItem
                                            ? (kpiItem.total_allocated_quantity ?? kpiItem.allocated_quantity)
                                            : (conversionData?.total_allocated_quantity && conversionData.total_allocated_quantity > 0
                                                ? conversionData.total_allocated_quantity
                                                : currentRemaining.reduce((sum, i) => sum + (Number(i.allocated_quantity ?? i.total_allocated_quantity) || 0), 0));

                                        const totalPicked = kpiItem
                                            ? kpiItem.picked_quantity
                                            : (conversionData?.total_picked_quantity && conversionData.total_picked_quantity > 0
                                                ? conversionData.total_picked_quantity
                                                : currentRemaining.reduce((sum, i) => sum + (Number(i.picked_quantity) || 0), 0));

                                        const totalRemaining = kpiItem
                                            ? kpiItem.pool_remaining
                                            : currentRemaining.reduce((sum, i) => sum + (Number(i.pool_remaining) || 0), 0);

                                        return (
                                            <>
                                                <div className="flex flex-col items-center">
                                                    <span className="text-[8px] font-black uppercase tracking-wider leading-tight text-muted-foreground/60">
                                                        {isSelected ? 'ALLOCATED' : 'TOTAL ALLOCATED'}
                                                    </span>
                                                    <span className="text-[11px] font-black leading-none text-foreground/90">
                                                        {totalAllocated}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex flex-col items-center">
                                                    <span className="text-[8px] font-black uppercase tracking-wider leading-tight text-blue-600/60 dark:text-blue-400/60">
                                                        PICKED
                                                    </span>
                                                    <span className="text-[11px] font-black leading-none text-blue-700 dark:text-blue-300">
                                                        {totalPicked}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex flex-col items-center">
                                                    <span className="text-[8px] font-black uppercase tracking-wider leading-tight text-primary/60">
                                                        REMAINING
                                                    </span>
                                                    <span className="text-[11px] font-black leading-none text-primary">
                                                        {totalRemaining}
                                                    </span>
                                                </div>
                                            </>
                ...
                {isLoadingDetails || !conversionData ? (
                    <div className="flex-1 min-h-[420px] flex flex-col items-center justify-center p-8 gap-3 bg-muted/5">
                        <motion.div
                        ...
                        >
                            ...
                            <div className="space-y-1">
                                <p className="text-xs font-black uppercase tracking-widest text-foreground">
                                    Loading Order & Inventory Details...
                                </p>
                                <p className="text-[11px] text-muted-foreground font-medium">
                                    Checking warehouse allocations, live on-hand quantities, and batches
                                </p>
                            </div>
                        </motion.div>
                    </div>
                ...
                <div className="flex-1 min-h-0 flex overflow-hidden bg-muted/5">
                    ...
                    <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                        <Tabs value={activeReceiptId} onValueChange={setActiveReceiptId} className="flex flex-col flex-1 min-h-0">
                            ...
                            {receipts.map((r, idx) => (
                                <TabsContent key={`${r.id}-${idx}`} value={r.id} className="flex-1 flex flex-col min-h-0 overflow-hidden m-0 p-0">
                                    <div className={cn(
                ...
                <DialogFooter className="p-6 bg-muted/30 border-t flex flex-col sm:flex-row justify-end gap-3 flex-shrink-0">
                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                        <Button
                            ...
                            disabled={!conversionData || isLoadingDetails || receipts.some(r => !r.is_void_reference && r.items.length > 0)}
                        >
                        ...
                        <Button
                            ...
                            disabled={isValidating || isLoadingDetails || !conversionData}
                        >
                            {isValidating ? (
            ...
            <DialogContent showCloseButton={false} className={cn("max-h-[95vh] overflow-y-auto p-0 border-none shadow-2xl bg-zinc-100 dark:bg-zinc-950 flex flex-col", previewDialogWidthClass)}>
                ...
                <DialogHeader className="p-4 bg-zinc-50 dark:bg-zinc-900/50 border-b border-zinc-200 dark:border-zinc-800 flex flex-row items-center justify-between flex-shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <Badge variant="outline" className="text-[9px] font-mono uppercase tracking-wider text-muted-foreground border-primary/20">
                            {isOfficialReceipt ? "Official Receipt (Full Sheet)" : "Thermal Receipt (58mm POS)"}
                        </Badge>
                    </div>

                    {/* Quick Receipt Switcher if multiple receipts */}
                    {receipts.filter(r => !r.is_void_reference).length > 1 && (
                        <div className="flex items-center gap-1.5 bg-muted/40 p-1 rounded-xl border border-border/50">
                            <Button
                                size="sm"
                                variant={previewFilterId === "ALL" ? "default" : "ghost"}
                                className={cn(
                                    "h-7 text-[10px] font-black px-3 rounded-lg transition-all",
                                    previewFilterId === "ALL" ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                                )}
                                onClick={() => setPreviewFilterId("ALL")}
                            >
                                All ({receipts.filter(r => !r.is_void_reference).length})
                            </Button>
                            {receipts.filter(r => !r.is_void_reference).map((r, idx) => (
                                <Button
                                    key={`${r.id}-${idx}`}
                                    size="sm"
                                    variant={previewFilterId === r.id ? "default" : "ghost"}
                                    className={cn(
                                        "h-7 text-[10px] font-bold px-2.5 rounded-lg max-w-[130px] truncate transition-all",
                                        previewFilterId === r.id ? "bg-primary text-white shadow-sm" : "text-muted-foreground hover:text-foreground"
                                    )}
                                    onClick={() => setPreviewFilterId(r.id)}
                                >
                                    {r.receipt_no || `Receipt ${idx + 1}`}
                                </Button>
                            ))}
                        </div>
                    )}
                ...
                <ScrollArea className="flex-1 min-h-0 bg-zinc-100/50 dark:bg-zinc-900/30 overflow-y-auto overflow-x-hidden">
                        <style>
                            {`
                                ...
                                /* Thermal Screen Styles (Comfortable, Modern POS Receipt) */
                                .thermal-paper-sheet {
                                    ...
                                    border-radius: 6px;
                                    box-shadow: 0 20px 45px -12px rgba(0, 0, 0, 0.35), 0 0 0 1px rgba(0, 0, 0, 0.08);
                                }
                                .thermal-receipt-area {
                                ...
                                .thermal-divider {
                                    border-top: 1px dashed #71717a !important;
                                    margin: 3mm 0 !important;
                                }

                                /* Preview Styles - Strict Border Free for Official Receipt */
                                ...
                                .table-right-col { padding-right: 0.3in !important; text-align: right !important; font-weight: normal; }
                        ...
                        <div className="preview-shell flex-col gap-8">
                            {isOfficialReceipt ? (
                                receipts
                                    ...
                                    .filter(r => previewFilterId === "ALL" || r.id === previewFilterId)
                                    .map((currentReceipt, receiptIdx, filteredArray) => (
                                <div
                                    key={`or-preview-${currentReceipt.id}-${receiptIdx}`}
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                {orTemplate ? (
                                                    Object.entries(orTemplate.fields).map(([key, config], fIdx) => {
                                ...
                                receipts
                                    ...
                                    .map((currentReceipt, receiptIdx, filteredArray) => (
                                     <div
                                        key={`thermal-preview-${currentReceipt.id}-${receiptIdx}`}
                                        style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 32 }}
                                    >
                                        {filteredArray.length > 1 && (
                                            <div className="mb-3 px-3 py-1 bg-primary/10 border border-primary/20 rounded-full text-[10px] font-black uppercase tracking-widest text-primary shadow-sm">
                                                {`Receipt ${receiptIdx + 1} of ${filteredArray.length} — ${currentReceipt.receipt_no || `Draft #${receiptIdx + 1}`}`}
                                            </div>
                                        )}
                                        <div
                                            ref={receiptIdx === 0 ? componentRef : undefined}
                                            id={receiptIdx === 0 ? "receipt-print-area" : `receipt-print-area-${receiptIdx}`}
                                            className="receipt-print-area thermal-paper-sheet thermal-preview-sheet bg-white text-zinc-900 font-mono shadow-2xl rounded-sm overflow-hidden"
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <pre className="thermal-pre">{buildThermalLines(currentReceipt).join("\n")}</pre>
                                            </div>
                                        </div>
                                    </div>
                                ))
