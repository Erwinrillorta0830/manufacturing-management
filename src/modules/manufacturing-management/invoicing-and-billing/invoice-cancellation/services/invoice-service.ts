...
export const InvoiceService = {
  ...
  async getInvoicesForCancellation(): Promise<SalesInvoice[]> {
    const res = await fetch("/api/manufacturing/invoicing-and-billing/invoice-cancellation");
  ...
  async getAllRequests(): Promise<{ data: CancellationRequest[], count: number }> {
    const res = await fetch("/api/manufacturing/invoicing-and-billing/invoice-cancellation-approval");
  ...
  async getReportData(page = 0, size = 10): Promise<{ content: CancellationReportDto[], totalElements: number }> {
    const res = await fetch(`/api/manufacturing/invoicing-and-billing/invoice-summary-report?page=${page}&size=${size}`);
