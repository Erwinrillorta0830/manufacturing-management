...
export default function ApprovalQueueModule() {
    ...
    const loadBatches = useCallback(async () => {
        ...
        try {
            ...
            console.error("Failed to load approval batches");
        } finally {
            setLoading(false);
        }
    ...
    return (
        <ConsolidationShell>
            <ConsolidationHeader
                ...
                title="Approval"
                accent="Queue"
                description="Review and approve batches that have completed picking."
                controls={
                    <div className="grid w-full gap-3 sm:grid-cols-2 lg:w-auto lg:grid-cols-[260px_320px]">
                        ...
                        <FilterField label="Search">
                            <div className="relative">
                                ...
                                <Input
                                    placeholder="Search batches for approval..."
            ...
            <ConsolidationSection eyebrow="Quality Review" title="Batches Awaiting Approval">
                {!selectedBranchId ? (
                    <ConsolidationEmptyState icon={Building2} title="Select Branch" description="Choose a branch to view its approval queue." />
                ) : loading ? (
                    <div className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4" aria-label="Loading approval batches">
                        {Array.from({ length: 8 }, (_, index) => <Skeleton key={index} className="h-48 rounded-xl" />)}
                    </div>
                ) : batches.length === 0 ? (
                    <ConsolidationEmptyState icon={ShieldCheck} title="No Batches Awaiting Approval" description="Batches in Picked status will appear here once picking is complete." />
                ) : (
