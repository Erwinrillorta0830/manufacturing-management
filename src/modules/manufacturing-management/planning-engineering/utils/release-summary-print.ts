 ...
 */
export interface ReleaseSummaryPrintOptions {
    /** Overrides the "Release Production Run Summary" document title/heading. Defaults to release wording. */
    documentTitle?: string;
}

export function buildReleaseSummaryHtml(data: ReleaseSummaryPrintData, options?: ReleaseSummaryPrintOptions): string {
    const documentTitle = options?.documentTitle?.trim() || "Release Production Run Summary";
    ...
    return `<html>
                <head>
                    <title>${escapeHtml(documentTitle)} - ${escapeHtml(data.joNumber)}</title>
                    <style>
                        ...
                        .header, .jo-summary, .footer { break-inside: avoid; }
                        .footer { border-top: 1px solid #e2e8f0; padding-top: 10px; font-size: 9px; color: #64748b; display: flex; justify-content: space-between; margin-top: 12px; }
                        @media print {
