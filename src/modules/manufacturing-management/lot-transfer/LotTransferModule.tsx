                        ...
                        <tbody className="divide-y">
                            {controller.records.map((record) => (
                                <tr key={record.id} className={controller.selectedId === record.id ? "bg-primary/5" : ""}>
                                    <td className="px-3 py-2.5 font-semibold">{record.requestNo}<br /><StatusBadge status={record.status} /><br /><span className="text-xs font-normal text-muted-foreground">Transfer: {formatDate(record.transferDate)} · {uomLabel(record.unitId, controller.lots)}</span></td>
...
function ApprovalQueue({ controller, onReview }: {
    ...
    return (
        <section className={panelClassName} aria-labelledby="lot-transfer-approval-queue-heading">
            ...
            {controller.records.length === 0 ? <EmptyState message="No lot-transfer requests are waiting for QA approval." /> : <div className="space-y-2">{controller.records.map((row) => <button type="button" key={row.id} onClick={() => onReview(row)} className={`w-full rounded-lg border p-3 text-left transition hover:border-primary ${controller.selectedId === row.id ? "border-primary bg-primary/5" : ""}`}><div className="flex items-center justify-between gap-2"><strong>{row.requestNo}</strong><StatusBadge status={row.status} /></div><div className="mt-2 text-xs text-muted-foreground">Transfer date: {formatDate(row.transferDate)} · UOM: {uomLabel(row.unitId, controller.lots)}</div><div className="mt-1 text-xs text-muted-foreground">Lot #{row.sourceLotId} / {row.sourceBatchNo} <ArrowRight className="mx-1 inline h-3 w-3" /> Lot #{row.targetLotId} / {row.targetBatchNo}</div><div className="mt-1 text-sm">Quantity: <strong>{formatQuantity(row.quantity)}</strong></div></button>)}</div>}
        </section>
    );
}
    ...
    return (
        <section className={panelClassName} aria-labelledby="lot-transfer-posting-review-heading">
            {!record ? <EmptyState message="Select an Approved request to post its paired inventory movements." /> : <>
                ...
                <div className="mt-4 rounded-lg border bg-muted/20 p-3 text-sm"><strong>Reason</strong><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{record.reason}</p></div>
                {notice && <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200">{notice}</div>}
                {record.status === "Approved" && <div className="mt-4 flex justify-end"><Button type="button" onClick={() => void handlePost()} disabled={controller.isActionLoading || !preview?.canPost}><Upload />Post transfer</Button></div>}
            </>}
        </section>
    );
}
...
function SummaryTable({ controller, onView }: {
    ...
    return (
        <section className={panelClassName} aria-labelledby="lot-transfer-summary-heading">
            ...
            {controller.records.length === 0 ? <EmptyState message="No lot-transfer records match the selected report filters." /> : <div className="overflow-x-auto rounded-lg border"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-muted/50 text-xs uppercase text-muted-foreground"><tr><th className="px-3 py-2.5">Request</th><th className="px-3 py-2.5">Product / branch</th><th className="px-3 py-2.5">Source -&gt; target</th><th className="px-3 py-2.5">Qty</th><th className="px-3 py-2.5">Decision</th><th className="px-3 py-2.5">Audit</th></tr></thead><tbody className="divide-y">{controller.records.map((row) => <tr key={row.id} className={controller.selectedId === row.id ? "bg-primary/5" : ""}><td className="px-3 py-2.5 font-semibold">{row.requestNo}<br /><span className="text-xs text-muted-foreground">Transfer: {formatDate(row.transferDate)}<br />Requested: {formatDate(row.requestedAt)}</span></td><td className="px-3 py-2.5">{productLabel(row.productId, controller.products)}<br /><span className="text-xs text-muted-foreground">{branchLabel(row.branchId, controller.branches)} · UOM {uomLabel(row.unitId, controller.lots)}</span></td><td className="px-3 py-2.5">{row.sourceBatchNo} <ArrowRight className="mx-1 inline h-3 w-3" /> {row.targetBatchNo}<br /><span className="text-xs text-muted-foreground">Lot #{row.sourceLotId} -&gt; Lot #{row.targetLotId}</span></td><td className="px-3 py-2.5">{formatQuantity(row.quantity)}</td><td className="px-3 py-2.5"><StatusBadge status={row.status} /></td><td className="px-3 py-2.5"><Button type="button" variant="outline" size="sm" onClick={() => onView(row)}><Eye />View</Button></td></tr>)}</tbody></table></div>}
        </section>
    );
}
