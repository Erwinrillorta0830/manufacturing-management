...
export async function fetchLotsByBranch(branchId?: number, token?: string): Promise<MMLot[]> {
  try {
    ...
    if (!res.ok) {
      console.warn(`[LotTracking] Failed to fetch mm_lots server-side: ${res.status}`);
      return [];
    }
...
export async function fetchInventoryLots(params: {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[LotTracking] Failed to fetch mm_inventory_lots server-side: ${res.status}`);
      return [];
    }
...
export function resolveProductClassification(
  ...
  const UNKNOWN = { code: "OTHER" as ProductClassification, label: "-" };
  ...
  if (typeof productType === "number") {
    ...
    const rawId = ptObj.id ?? ptObj.product_type_id ?? ptObj.type_id;
    if (rawId !== undefined && rawId !== null) {
      typeId = Number(rawId);
    }
    typeName = String(ptObj.name ?? ptObj.type_name ?? ptObj.description ?? "");
  } else if (typeof productType === "string") {
    typeName = productType;
  }

  // 1. Exact product_type ID — authoritative, no fallback
  ...
  // 2. Exact type name from API object (case-sensitive, matches Directus values)
  if (typeName === "Raw Materials")   return { code: "RM",  label: "Raw Materials" };
  if (typeName === "Packaging Items") return { code: "PKG", label: "Packaging Items" };
  if (typeName === "Finished Goods")  return { code: "FG",  label: "Finished Goods" };

  // No match — show "-" in UI, no guessing
  return UNKNOWN;
}
