    ...
    const loadData = async () => {
      ...
      try {
        ...
        (lotsData || []).forEach((lot) => {
          ...
          const activeOnhandBatches = onhandForLot.filter((bo) => Number(bo.onhandQuantity || 0) > 0);
          let lotStock = 0;
          let activeBatchesCount = 0;

          if (onhandForLot.length > 0) {
            lotStock = activeOnhandBatches.reduce((sum, bo) => sum + Number(bo.onhandQuantity || 0), 0);
            activeBatchesCount = activeOnhandBatches.length;
          } else if (invLotsForLot.length > 0) {
            const activeInvLots = invLotsForLot.filter((ib) => Number(ib.available_quantity || 0) > 0);
            lotStock = activeInvLots.reduce((sum, ib) => sum + Number(ib.available_quantity || 0), 0);
            activeBatchesCount = activeInvLots.length;
          }

          bCountMap.set(lId, activeBatchesCount);
        ...
        (lotsData || []).forEach((lot) => {
          ...
          // Enrich existing active products with mm_inventory_lots metadata
          invLotsForLot.forEach((ib) => {
            const pId = getProductId(ib);
            if (pId > 0) {
              const existing = productQtyMap.get(pId);
              if (existing) {
                const prodObj = typeof ib.product_id === 'object' && ib.product_id !== null ? (ib.product_id as Record<string, unknown>) : null;
                const meta = productMetaMap.get(pId);
                if (!existing.name && (ib.product_name || prodObj?.product_name || meta?.name)) {
                  existing.name = ib.product_name || (prodObj?.product_name as string) || meta?.name;
                }
                if (!existing.code && (ib.product_code || prodObj?.product_code || meta?.code)) {
                  existing.code = ib.product_code || (prodObj?.product_code as string) || meta?.code;
                }
                if (!existing.type && (ib.product_type || prodObj?.product_type || meta?.type)) {
                  existing.type = ib.product_type || prodObj?.product_type || meta?.type;
                }
                const rawCat = ib.category_name || (typeof prodObj?.product_category === 'object' ? (prodObj.product_category as { category_name?: string })?.category_name : String(prodObj?.product_category || '')) || (meta?.cat ? String(meta.cat) : null);
                if (!existing.cat && rawCat) {
                  existing.cat = rawCat;
                }
              }
          ...
          storedMap.set(lId, {
            ...
            active_batch_count: isEmpty ? 0 : (batchCount || storedItems.length),
