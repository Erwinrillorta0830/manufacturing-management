...
export function useFinishedGoods(initialTab: string = "details") {
            ...
            const res = await registerProduct(
                {
                    ...
                    has_bom: registerForm.hasBom ? 1 : 0
                },
    ...
    const handleSave = async () => {
        ...
        const editValidationInput = {
            ...
            hasBom: editedDetails.has_bom !== undefined
                ? Boolean(editedDetails.has_bom)
                : (selectedProduct?.has_bom !== undefined
                    ? Boolean(selectedProduct.has_bom)
                    : selectedProduct?.has_versions !== false),
            has_bom: editedDetails.has_bom !== undefined
                ? Boolean(editedDetails.has_bom)
                : (selectedProduct?.has_bom !== undefined
                    ? Boolean(selectedProduct.has_bom)
                    : selectedProduct?.has_versions !== false),
        ...
        try {
            const detailsPayload = {
                ...
                has_bom: (editedDetails.has_bom !== undefined ? Boolean(editedDetails.has_bom) : (selectedProduct?.has_bom !== undefined ? Boolean(selectedProduct.has_bom) : selectedProduct?.has_versions !== false)) ? 1 : 0,
            ...
            if (saveSucceeded) {
                ...
                const finalHasBom = editedDetails.has_bom !== undefined
                    ? Boolean(editedDetails.has_bom)
                    : (selectedProduct?.has_bom !== undefined
                        ? Boolean(selectedProduct.has_bom)
                        : selectedProduct?.has_versions !== false);
