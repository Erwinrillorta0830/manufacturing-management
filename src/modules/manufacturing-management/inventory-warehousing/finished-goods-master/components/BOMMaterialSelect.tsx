...
export function BOMMaterialSelect({
    ...
    React.useEffect(() => {
        ...
        async function fetchSelected() {
            try {
                ...
                const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/products?search=${value}&limit=1&excludeRollup=true`, {
    ...
    React.useEffect(() => {
        ...
        async function loadDefaults() {
            setLoading(true);
            try {
                const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/products?limit=50&excludeRollup=true", {
                    signal: controller.signal
                });
                if (res.ok) {
                    const data = await res.json();
    ...
    React.useEffect(() => {
        ...
        async function searchMaterials() {
            setLoading(true);
            try {
                const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/products?search=${encodeURIComponent(debouncedSearch.trim())}&limit=50&excludeRollup=true`, {
                    signal: controller.signal
                });
                if (res.ok) {
                    const data = await res.json();
