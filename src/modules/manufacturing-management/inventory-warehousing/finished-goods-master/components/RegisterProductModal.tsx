                ...
                <form noValidate onSubmit={(e) => handleRegisterProduct(e, registrationType)} className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6">
                    ...
                    <div className="bg-muted/10 border border-border/40 rounded-xl p-5 space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label className="text-[11px] font-bold text-muted-foreground uppercase block mb-1">Maintaining Quantity <span className="text-red-500">*</span></label>
                                <input
                                    id="register-maintainingQuantity"
                                    type="number"
                                    step="0.01"
                                    min="0"
                                    placeholder="e.g. 100"
                                    value={registerForm.maintainingQuantity}
                                    onFocus={(e) => {
                                        if (e.target.value === "0") {
                                            updateRegisterField("maintainingQuantity", "");
                                        } else {
                                            e.target.select();
                                        }
                                    }}
                                    ...
                                    onChange={e => updateRegisterField("maintainingQuantity", e.target.value)}
                                    onBlur={(e) => {
                                        if (e.target.value.trim() === "" || isNaN(Number(e.target.value)) || Number(e.target.value) < 0) {
                                            updateRegisterField("maintainingQuantity", "0");
                                        }
                                    }}
                                    aria-invalid={!!registerError("maintainingQuantity")}
                                    aria-describedby={registerError("maintainingQuantity") ? "register-maintainingQuantity-error" : undefined}
                                    className={registerInputClass("maintainingQuantity")}
                                />
                                {registerErrorMessage("maintainingQuantity")}
                            </div>
                            {registerForm.hasBom !== false && (
                                <div>
                    ...
                    {/* Group 4: BOM Specification & Version */}
                    <div className="bg-muted/10 border border-border/40 rounded-xl p-5 space-y-4">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-border/40">
                            <div>
                                <h4 className="text-xs font-bold uppercase tracking-wider text-primary flex items-center gap-1.5">
                                    <Layers className="h-4 w-4" /> 4. BOM Specification &amp; Version
                                </h4>
                                <p className="text-[11px] text-muted-foreground mt-0.5">
                                    Enable to configure recipe bill of materials, workstation routes, and labor standards.
                                </p>
                            </div>
                            <label className="relative inline-flex items-center cursor-pointer shrink-0">
                                <input
                                    type="checkbox"
                                    checked={registerForm.hasBom !== false}
                                    onChange={(e) => {
                                        setRegisterForm(prev => ({ ...prev, hasBom: e.target.checked }));
                                        if (!e.target.checked) {
                                            clearRegisterFormError("versionName");
                                            clearRegisterFormError("expectedYield");
                                        }
                                    }}
                                    className="sr-only peer"
                                />
                                <div className="w-9 h-5 bg-muted peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-border after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-primary"></div>
                                <span className="ml-2.5 text-xs font-bold text-foreground select-none">
                                    {registerForm.hasBom !== false ? "Has BOM Recipe" : "No BOM (Direct Good)"}
                                </span>
                            </label>
                        </div>
                        ...
                        {registerForm.hasBom !== false ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2">
                                    <label className="text-[11px] font-bold text-muted-foreground uppercase block mb-1">
                                        Initial Version Name <span className="text-red-500">*</span>
                                    </label>
                                    <input
                            ...
                            <div className="p-3.5 rounded-xl bg-muted/40 border border-dashed border-border/60 text-xs text-muted-foreground flex items-center gap-2">
                                <Layers className="h-4 w-4 text-muted-foreground/60 shrink-0" />
                                <span>This finished good will be registered directly without a BOM version or workstation routing.</span>
                            </div>
                        )}
                    </div>
