...
export async function fetchHistoricalYield(productId: string | number): Promise<HistoricalYieldData> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/versions/historical-yield?productId=${productId}`);
    if (!res.ok) {
...
export async function applyHistoricalYield(versionId: number, expectedYieldPercentage: number): Promise<void> {
    const applyRes = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/versions/historical-yield`, {
        method: "POST",
