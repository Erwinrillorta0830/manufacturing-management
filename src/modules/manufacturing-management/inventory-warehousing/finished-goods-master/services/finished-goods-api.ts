...
export async function fetchProducts(search?: string, limit: number = 100): Promise<Product[]> {
    ...
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/products?${query.toString()}`);
    ...
    return data.map((p: BFFCatalogProduct) => {
        ...
        return {
            ...
            maintaining_quantity: p.maintaining_quantity !== undefined && p.maintaining_quantity !== null ? Number(p.maintaining_quantity) : null,
            ...
            has_bom: p.has_bom !== undefined && p.has_bom !== null ? Boolean(p.has_bom) : !!p.has_versions
        };
    });
}

export async function fetchBrands(): Promise<Brand[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/brands");
    if (!res.ok) throw new Error("Failed to fetch brands from BFF");
    return res.json();
}

export async function fetchCategories(): Promise<Category[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/categories");
    if (!res.ok) throw new Error("Failed to fetch categories from BFF");
    return res.json();
}

export async function fetchUnits(): Promise<Unit[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/units");
    if (!res.ok) throw new Error("Failed to fetch units from BFF");
    return res.json();
}

export async function fetchClasses(): Promise<ProductClass[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/classes");
    if (!res.ok) throw new Error("Failed to fetch classes from BFF");
    return res.json();
}

export async function fetchSegments(): Promise<ProductSegment[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/segments");
    if (!res.ok) throw new Error("Failed to fetch segments from BFF");
    return res.json();
}

export async function fetchSections(): Promise<ProductSection[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/sections");
...
export async function fetchVersions(productId: number): Promise<ProductVersion[]> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/versions?productId=${productId}`, { cache: "no-store" });
...
export async function fetchBOMDetails(productId: number, versionId: number, forexRate?: number): Promise<ProductVersion | null> {
    ...
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/bom-details?${query.toString()}`, { cache: "no-store" });
...
export async function saveBOMDetails(
    ...
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/bom-details", {
        method: "POST",
...
export async function updateProduct(
    ...
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/products", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product_id: productId, ...updates })
    });
    if (!res.ok) {
        let msg = "Failed to update product via BFF";
        try {
            const errJson = await res.json();
            if (errJson && errJson.error) msg = errJson.error;
        } catch {}
...
export async function registerProduct(
    ...
    uomId?: number,
    hasBom: boolean = true
): Promise<{ success: boolean; productId: number; version: ProductVersion | null }> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/products", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productDetails, versionName, supplierIds, expectedYield, baseQuantity, uomId, hasBom })
    });
    if (!res.ok) {
...
export async function registerNewVersion(
    ...
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/versions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
export async function fetchQATemplates(): Promise<QATemplate[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/qa-templates");
...
export async function saveQATemplate(templateId: number, template: Partial<QATemplate>): Promise<{ success: boolean; template: QATemplate }> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/qa-templates/${templateId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(template)
    });
...
export async function fetchAssets(): Promise<AssetRecord[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/assets");
    if (!res.ok) throw new Error("Failed to fetch assets from BFF");
    return res.json();
}

export async function fetchDepartments(): Promise<DepartmentRecord[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/departments");
...
export async function saveAsset(assetId: number, asset: Partial<AssetRecord>): Promise<{ success: boolean; asset: AssetRecord }> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/assets/${assetId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(asset)
    });
...
export async function fetchItems(): Promise<any[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/items");
...
export async function fetchItemTypes(): Promise<any[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/item-types");
...
export async function fetchItemClassifications(): Promise<any[]> {
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/item-classifications");
...
export async function fetchActiveVersionDraft(versionId: number): Promise<any | null> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/versions/drafts?versionId=${versionId}`, {
        cache: "no-store"
    });
...
export async function fetchActiveDraftsForProduct(productId: number): Promise<any[]> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/finished-goods-master/versions/drafts?productId=${productId}`, {
        cache: "no-store"
    });
    if (!res.ok) return [];
    const json = await res.json();
...
export async function initiateVersionDraft(
    ...
    const res = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/versions/drafts", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
