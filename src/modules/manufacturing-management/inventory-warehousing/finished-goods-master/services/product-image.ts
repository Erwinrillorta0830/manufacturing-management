...
export async function uploadProductImage(file: File): Promise<string> {
    ...
    const response = await fetch("/api/manufacturing/inventory-warehousing/finished-goods-master/files", {
        method: "POST",
        body: formData,
    });
...
export function getProductImageUrl(fileId: string | number): string {
    return `/api/manufacturing/inventory-warehousing/finished-goods-master/files?id=${encodeURIComponent(String(fileId))}`;
}
