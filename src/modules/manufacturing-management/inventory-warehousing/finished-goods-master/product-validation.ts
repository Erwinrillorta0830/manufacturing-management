...
function addEditValidationErrors(input: unknown): ProductValidationFields {
    ...
    const hasBom = details.has_bom !== undefined
        ? Boolean(details.has_bom)
        : (details.hasBom !== false && details.has_versions !== false);
    if (hasBom) {
    ...
    const maintainingQuantity = readNumber(details.maintaining_quantity ?? details.maintainingQuantity);
    if (maintainingQuantity === null || maintainingQuantity < 0) {
        fields.maintainingQuantity = "Maintaining Quantity is required and must be 0 or greater.";
    }
...
export function validateProductEditDetails(input: unknown): ValidatedProductEditDetails {
    ...
    return {
        ...
        expectedYield: (readNumber(details.expected_yield_percentage ?? details.expectedYieldPercent) ?? 100) as number,
        productShelfLife: readNumber(details.productShelfLife) as number,
        maintainingQuantity: (readNumber(details.maintaining_quantity ?? details.maintainingQuantity) ?? 0) as number,
        hasBom
    };
}
...
function addRegistrationValidationErrors(input: unknown): ProductValidationFields {
    ...
    const maintainingQuantity = readNumber(productDetails.maintaining_quantity ?? productDetails.maintainingQuantity);
    if (maintainingQuantity === null || maintainingQuantity < 0) {
        fields.maintainingQuantity = "Maintaining Quantity is required and must be 0 or greater.";
    }

    const hasBom = body.has_bom !== undefined
        ? Boolean(body.has_bom)
        : body.hasBom !== false;
    if (hasBom) {
...
export function validateProductRegistration(input: unknown): ValidatedProductRegistrationDetails {
    ...
    const resolvedVersionName = hasBom ? (providedVersion || (productCode ? `${productCode} Rev 1` : "Rev 1")) : undefined;
    const parsedYield = readNumber(body.expectedYield);
    const resolvedExpectedYield = hasBom ? (parsedYield ?? 100) : undefined;
    const maintainingQuantity = (readNumber(productDetails.maintaining_quantity ?? productDetails.maintainingQuantity) ?? 0) as number;
