...
export async function fetchBranches(): Promise<{
    ...
    const res = await fetch("/api/manufacturing/inventory-warehousing/branch-management");
...
export async function deleteBranch(id: number): Promise<unknown> {
    const res = await fetch(`/api/manufacturing/inventory-warehousing/branch-management?id=${id}`, {
        method: "DELETE",
    });
    if (!res.ok) {
        const err = await res.json();
