...
export const stockAdjustmentManualService = {
  ...
  async fetchAllHeaders(params?: { search?: string; branchId?: number; type?: string; status?: string }) {
    ...
    const res = await directusFetch<{ data: StockAdjustmentManualHeader[] }>(`${DIRECTUS_URL}/items/mm_stock_adjustment_header?${query}`);
  ...
  async fetchById(id: number): Promise<StockAdjustmentManualDetail> {
    ...
    const productGroupMap = new Map<number, StockAdjustmentManualItem>();
    items.forEach((item: RawItem) => {
      const pId = Number(typeof item.product_id === 'object' && item.product_id !== null ? (item.product_id?.product_id || item.product_id?.id) : item.product_id);
      ...
      if (!existing) {
        ...
        } as unknown as StockAdjustmentManualItem);
      } else {
    ...
    return {
      ...header,
      remarks: cleanedRemarks,
      items: assembledItems.length > 0 ? assembledItems : items,
      amount: totalAmount > 0 ? totalAmount : (Number(header.amount) || 0),
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      supplier_id: resolvedSupplier as any,
  ...
  async create(payload: { header: Record<string, unknown>; items: StockAdjustmentManualItem[]; userId?: number }) {
          ...
          for (const b of (group.batches || []) as any[]) {
            ...
            if (bQty > 0) {
              explodedItems.push({
                ...
                qa_status: (b.qa_status as "GOOD" | "DAMAGED" | "QUARANTINED" | "EXPIRED") || item.qa_status,
                allocations: undefined,
                allocation_plan: undefined,
        ...
        for (const alloc of item.allocations) {
          ...
          if (allocQty > 0) {
            explodedItems.push({
              ...
              lot_name: alloc.lot_name || item.lot_name,
              ...
              quantity: allocQty,
              manufacturing_date: alloc.manufacturing_date || item.manufacturing_date,
              expiry_date: alloc.expiry_date || item.expiry_date,
              cost_per_unit: alloc.unit_cost !== undefined ? Number(alloc.unit_cost) : item.cost_per_unit,
              qa_status: (alloc.qa_status as "GOOD" | "DAMAGED" | "QUARANTINED" | "EXPIRED") || item.qa_status,
              allocations: undefined,
              allocation_plan: undefined,
              lot_allocations: undefined,
            });
    ...
    const itemsPayload = await Promise.all(
      explodedItems.map(async (item: StockAdjustmentManualItem) => {
        ...
        if (!resolvedInventoryLotId && item.batch_no) {
          const syncRes = await syncInventoryLotBatch({
            ...
            inventory_condition: item.qa_status,
        ...
        return {
          ...
          inventory_condition: item.qa_status || "GOOD",
          source_type: "MANUAL",
          remarks: itemRemarks || "Stock Adjustment",
    ...
    const newItemsRes = await directusFetch<{ data: Array<{ id: number }> | { id: number } }>(`${DIRECTUS_URL}/items/mm_stock_adjustment?fields=id`, {
      method: "POST",
  ...
  async update(id: number, payload: { header: Record<string, unknown>; items: StockAdjustmentManualItem[]; userId?: number }) {
    ...
    const existingItemsRes = await directusFetch<{ data: { id: number }[] }>(
      `${DIRECTUS_URL}/items/mm_stock_adjustment?filter={"doc_no":{"_eq":"${payload.header.doc_no}"}}&fields=id&limit=-1`
    );
    ...
    if (itemIds.length > 0) {
      try {
        const existingAttRes = await directusFetch<{ data: { id: number }[] }>(
          `${DIRECTUS_URL}/items/mm_stock_adjustment_attachment?filter={"stock_adjustment_id":{"_in":${JSON.stringify(itemIds)}}}&fields=id&limit=-1`
        );
   ...
   * NOTE: Stock Adjustment does NOT create mm_inventory_lots rows.
   * Posting generates an IN or OUT movement in the inventory ledger.
   */
  ...
  async deleteStockAdjustment(id: number, userId?: number) {
    ...
    if (docNo) {
      const itemsRes = await directusFetch<{ data: { id: number }[] }>(
        `${DIRECTUS_URL}/items/mm_stock_adjustment?filter={"doc_no":{"_eq":"${docNo}"}}&fields=id&limit=-1`
      );
      const itemIds = itemsRes.data.map((i: { id: number }) => i.id);

      // Clean up attachments and items for draft
      if (itemIds.length > 0) {
        try {
   ...
   */
  async fetchBranches() {
    const res = await directusFetch<{ data: { id: number; branch_name: string; branch_code: string; isBadStock?: number | boolean | string | null; bad_stock_branch_id?: number | null; isActive?: number | boolean | string | null }[] }>(`${DIRECTUS_URL}/items/branches?filter[isActive][_eq]=1&fields=id,branch_name,branch_code,isBadStock,bad_stock_branch_id,isActive&sort=branch_name&limit=-1`);
    return (res.data || []).filter(
      (b) => b.isActive === 1 || b.isActive === true || b.isActive === "1"
    );
  },
  ...
  /**
   * Fetch Finished Goods products without requiring a supplier filter.
   */
  async fetchFinishedGoodsProducts(search?: string) {
    const filters: Record<string, unknown> = {
      _and: [
        { isActive: { _eq: 1 } },
        { product_type: { _eq: 388 } },
      ],
    };
  ...
  async fetchProductById(productId: number): Promise<StockAdjustmentManualProduct | null> {
    try {
      const res = await directusFetch<{ data: Record<string, unknown> }>(
        `${DIRECTUS_URL}/items/products/${productId}?fields=product_id,product_name,product_code,price_per_unit,cost_per_unit,barcode,description,unit_of_measurement,unit_of_measurement.unit_id,unit_of_measurement.unit_name,unit_of_measurement.order,product_brand.brand_name`
      );
      ...
      const uomObj = typeof uom === 'object' && uom !== null ? uom : undefined;
      const brand = p['product_brand'] as Record<string, unknown> | undefined;
      const resolvedUnitId = uomObj ? (uomObj['unit_id'] || uomObj['id']) : (typeof uom === 'number' ? uom : undefined);
      return {
        ...p,
