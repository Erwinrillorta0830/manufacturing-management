...
export const stockAdjustmentService = {
  ...
  async fetchBranchInventory(branchId: number, token: string): Promise<{ product_id: number; running_inventory: number }[]> {
    try {
      ...
      try {
        ...
        console.warn("Spring running inventory timed out or failed:", err);
      }
      ...
      try {
        ...
        console.warn("Spring RFID on-hand timed out or failed:", err);
      }
  ...
  async fetchProducts(params?: { search?: string }) {
    let query = `fields=product_id,product_name,product_code,price_per_unit,cost_per_unit,barcode,description,product_image,unit_of_measurement.unit_name,unit_of_measurement.order,product_brand.brand_name&limit=100&sort=product_name`;
  ...
  async fetchProductsBySupplier(supplierId: number, search?: string) {
    ...
    const query = `fields=product_id,product_name,product_code,price_per_unit,cost_per_unit,barcode,description,product_image,unit_of_measurement.unit_name,unit_of_measurement.order,product_brand.brand_name&limit=500&sort=product_name&filter=${JSON.stringify(filters)}`;
    const res = await directusFetch<{ data: unknown[] }>(`${DIRECTUS_URL}/items/products?${query}`);
    const products = res.data || [];
  ...
  async fetchProductById(productId: number) {
    const query = `fields=product_id,product_name,product_code,price_per_unit,cost_per_unit,barcode,description,product_image,unit_of_measurement.unit_name,unit_of_measurement.order,product_brand.brand_name`;
