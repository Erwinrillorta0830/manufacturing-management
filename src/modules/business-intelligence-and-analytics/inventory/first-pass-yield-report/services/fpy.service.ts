...
export async function getFPYReportData(): Promise<{
    ...
    const rejectionReasonMap = new Map<number, { code: string; name: string; category?: string }>();
    rejectionReasonsRaw.forEach((r: any) => {
    ...
    const inspectionLogsByJO = new Map<number, any[]>();
    inspectionLogsRaw.forEach((log: any) => {
    ...
    const yieldLedgersByJO = new Map<number, any[]>();
    yieldLedgersRaw.forEach((yl: any) => {
    ...
    const reworkOrdersByParent = new Map<number, any[]>();
    jobOrdersRaw.forEach((jo: any) => {
    ...
    const rows: FPYReportRow[] = jobOrdersRaw.map((jo: any) => {
        ...
        if (linkedReworks.length > 0) {
            const linkedReworkQty = linkedReworks.reduce((sum: number, r: any) => sum + Number(r.target_quantity || 0), 0);
        ...
        const linkedReworkNos = linkedReworks.map((r: any) => r.job_order_no);
    ...
    const uniqueBranches = branchesRaw.map((b: any) => ({
        id: b.id,
        label: b.branch_name || `Branch #${b.id}`,
        code: b.branch_code
    }));

    const uniqueProducts = productsRaw.map((p: any) => ({
        id: p.product_id,
        label: p.product_name || `Product #${p.product_id}`,
        code: p.product_code
    }));

    const rejectionReasons = rejectionReasonsRaw.map((r: any) => ({
        id: r.id,
...
export async function getJobOrderFPYBreakdown(jobOrderId: number): Promise<FPYDetailBreakdown | null> {
    ...
    const product = productsRaw.find((p: any) => p.product_id === jo.product_id);
    const branch = branchesRaw.find((b: any) => b.id === jo.branch_id);
    ...
    const inspectionLogs: InspectionLogItem[] = inspectionLogsRaw.map((log: any) => {
        ...
        return {
            ...
            rejection_reason_id: log.rejection_reason_id,
    ...
    // Format route steps
    const routeSteps: RouteStepItem[] = routesRaw.map((r: any) => ({
        jo_route_id: r.jo_route_id,
        sequence_order: r.sequence_order,
        work_center_name: workCenterMap.get(r.work_center_id) || `Station #${r.work_center_id}`,
        operation_name: operationMap.get(r.operation_id) || `Operation #${r.operation_id}`,
    ...
    const qaRecords: ParameterQARecord[] = qaRecordsRaw.map((rec: any) => ({
        qa_record_id: rec.qa_record_id,
        jo_route_id: rec.jo_route_id,
        parameter_id: rec.parameter_id,
        parameter_name: paramMap.get(rec.parameter_id) || `Parameter #${rec.parameter_id}`,
        value_text: rec.value_text,
        value_numeric: rec.value_numeric !== null ? Number(rec.value_numeric) : null,
        value_boolean: rec.value_boolean !== null ? Boolean(rec.value_boolean) : null,
    ...
    const yieldLedgers: ShiftYieldLedgerItem[] = yieldLedgersRaw.map((yl: any) => ({
        ...
        lot_number: yl.lot_number,
        logged_at: yl.logged_at,
        production_date: yl.production_date,
        remarks: yl.remarks
    }));

    // Find linked rework orders
    const reworkOrders: LinkedReworkOrderItem[] = allJobOrdersRaw
        .filter((o: any) => Number(o.parent_job_order_id) === jobOrderId)
        .map((o: any) => ({
