...
} from "@/modules/manufacturing-management/inventory-warehousing/finished-goods-master/product-validation";
...
export async function GET(request: Request) {
    try {
        ...
        const resolvedProducts = await Promise.all(catalogProducts.map(async (p: DirectusProduct) => {
            ...
            productCopy.has_bom = p.has_bom !== undefined && p.has_bom !== null ? Boolean(p.has_bom) : productCopy.has_versions;
...
export async function POST(request: Request) {
    try {
        ...
        const productPayload = {
            ...
            isActive: 1,
            has_bom: validatedDetails.hasBom ? 1 : 0,
            status: "Active",
            item_type: "regular",
            product_type: 388,
