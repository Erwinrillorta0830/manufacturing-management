...
function normalizeRfid(raw: string): string {
    const value = toStr(raw).toUpperCase();
    return new RegExp(`^[0-9A-F]{${RFID_LEN}}$`).test(value) ? value : "";
}
...
const RFID_ELIGIBLE_INVENTORY_STATUSES = new Set([3, 9, 11]);
...
function isPreQaRfidAnchor(row: PORow) {
    const receivingMethod = toStr(row.receiving_method).toLowerCase();
    return (!receivingMethod || receivingMethod === "rfid")
        && toNum(row.isPosted) !== 1
        && toNum(row.is_reverted) !== 1
        && toNum(row.received_quantity) === 0
        && !toStr(row.receipt_no)
        && !toStr(row.receipt_date)
        && !toStr(row.received_date);
}

function isExtraPoLine(row: Record<string, unknown>) {
    const explicit = row.is_ExtraItem ?? row.is_extra_item;
    return ["1", "true"].includes(String(explicit).toLowerCase()) || toNum(row.ordered_quantity) === 0;
}

async function cleanupEmptyPreQaRfidAnchor(base: string, porId: number) {
    const anchorResponse = await fetchJson<{ data: PORow }>(
        `${base}/items/${POR_COLLECTION}/${porId}?fields=${encodeURIComponent(POR_SAFE_FIELDS)}`
    ).catch(() => null);
    const anchor = anchorResponse?.data;
    if (!anchor || !isPreQaRfidAnchor(anchor)) return false;

    const tagsResponse = await fetchJson<{ data: Record<string, unknown>[] }>(
        `${base}/items/${POR_ITEMS_COLLECTION}?limit=1&filter[purchase_order_product_id][_eq]=${porId}&fields=receiving_item_id`
    );
    if ((tagsResponse.data || []).length > 0) return false;

    await fetchJson(`${base}/items/${POR_COLLECTION}/${porId}`, { method: "DELETE" });
    return true;
}

async function findTaggedPreQaRfidAnchor(base: string, poId: number, productId: number, branchId: number) {
    const candidates = await fetchJson<{ data: PORow[] }>(
        `${base}/items/${POR_COLLECTION}?limit=-1&filter[purchase_order_id][_eq]=${encodeURIComponent(String(poId))}&filter[product_id][_eq]=${encodeURIComponent(String(productId))}&filter[branch_id][_eq]=${encodeURIComponent(String(branchId))}&filter[isPosted][_eq]=0&filter[is_reverted][_eq]=0&fields=${encodeURIComponent(POR_SAFE_FIELDS)}`
    );

    for (const candidate of candidates.data || []) {
        if (!isPreQaRfidAnchor(candidate)) continue;
        const tags = await fetchJson<{ data: Record<string, unknown>[] }>(
            `${base}/items/${POR_ITEMS_COLLECTION}?limit=1&filter[purchase_order_product_id][_eq]=${encodeURIComponent(String(candidate.purchase_order_product_id))}&fields=receiving_item_id`
        );
        if ((tags.data || []).length > 0) return candidate;
    }
...
async function ensureOpenReceivingRow(args: {
    ...
    const existingAnchor = await findTaggedPreQaRfidAnchor(base, poId, productId, branchId);
    if (existingAnchor?.purchase_order_product_id) {
        return { porId: toNum(existingAnchor.purchase_order_product_id), receivedQty: toNum(existingAnchor.received_quantity), created: false };
    }
...
export async function GET() {
    try {
        await requirePurchaseOrderModuleAccess({ modulePath: PURCHASE_ORDER_MODULE_PATHS.rfidReceiving });
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === "save_receipt") {
            return bad("RFID tagging is a pre-QA step. Complete receipt details in QA Receiving.", 409);
        }
        ...
        if (action === "add_extra_product") {
            ...
            if (!poId || !productId || !branchId) return bad("Missing poId, productId, or branchId", 400);

            const poUrl = `${base}/items/${PO_COLLECTION}/${poId}?fields=supplier_name,inventory_status,discount_type.*,discount_type.line_per_discount_type.line_id.*,vat_amount,withholding_tax_amount`;
            const pj = await fetchJson<{ data: POHeaderRow }>(poUrl).catch(() => null);
            const po = pj?.data;
            if (!po) return bad("PO not found", 404);
            if (!RFID_ELIGIBLE_INVENTORY_STATUSES.has(toNum(po.inventory_status))) {
                return bad("This purchase order is no longer eligible for RFID receiving.", 409);
            }

            const existingLines = await fetchPOProductsByPOId(base, poId);
            if (!existingLines.some(line => toNum(line.branch_id) === branchId)) {
                return bad("The selected branch is not assigned to this purchase order.", 409);
            }
            ...
            if (!productJ?.data) return bad("Product not found", 404);
            ...
            if (!productLinksMap.has(productId)) {
                return bad("The selected product is not approved for this supplier.", 409);
            }
        ...
        if (action === "delete_extra_product") {
            const requestedId = toNum(body.porId);
            if (!requestedId) return bad("Missing porId", 400);

            let productId = requestedId;
            let extraProduct = await fetchJson<{ data: Record<string, unknown> }>(
                `${base}/items/${PO_PRODUCTS_COLLECTION}/${requestedId}?fields=purchase_order_product_id,purchase_order_id,product_id,branch_id,ordered_quantity`
            ).catch(() => null);

            // The UI normally has a POR id after the first scan. Resolve that
            // back to the extra PO line before deleting anything.
            if (!extraProduct?.data || !isExtraPoLine(extraProduct.data)) {
                const receiving = await fetchJson<{ data: Record<string, unknown> }>(
                    `${base}/items/${POR_COLLECTION}/${requestedId}?fields=purchase_order_id,product_id,branch_id`
                ).catch(() => null);
                if (receiving?.data) {
                    const params = new URLSearchParams({
                        limit: "1",
                        fields: "purchase_order_product_id,purchase_order_id,product_id,branch_id,ordered_quantity",
                        "filter[purchase_order_id][_eq]": String(toNum(receiving.data.purchase_order_id)),
                        "filter[product_id][_eq]": String(toNum(receiving.data.product_id)),
                        "filter[branch_id][_eq]": String(toNum(receiving.data.branch_id)),
                        "filter[ordered_quantity][_eq]": "0"
                    });
                    extraProduct = await fetchJson<{ data: Record<string, unknown>[] }>(
                        `${base}/items/${PO_PRODUCTS_COLLECTION}?${params.toString()}`
                    ).then(result => ({ data: result.data?.[0] })).catch(() => null);
                }
            }
            ...
            productId = toNum(extraProduct.data.purchase_order_product_id);
            const poId = toNum(extraProduct.data.purchase_order_id);
            const extraProductId = toNum(extraProduct.data.product_id);
            const branchId = toNum(extraProduct.data.branch_id);
            const receivingRows = await fetchJson<{ data: PORow[] }>(
                `${base}/items/${POR_COLLECTION}?limit=-1&filter[purchase_order_id][_eq]=${poId}&filter[product_id][_eq]=${extraProductId}&filter[branch_id][_eq]=${branchId}&filter[isPosted][_eq]=0&filter[receipt_no][_null]=true&fields=${encodeURIComponent(POR_SAFE_FIELDS)}`
            ).catch(() => ({ data: [] }));
            for (const receivingRow of receivingRows.data || []) {
                if (!isPreQaRfidAnchor(receivingRow)) continue;
                const tags = await fetchJson<{ data: Record<string, unknown>[] }>(
                    `${base}/items/${POR_ITEMS_COLLECTION}?limit=1&filter[purchase_order_product_id][_eq]=${toNum(receivingRow.purchase_order_product_id)}&fields=receiving_item_id`
                );
                if ((tags.data || []).length > 0) return bad("Remove the RFID tags before deleting this extra product.", 409);
                await cleanupEmptyPreQaRfidAnchor(base, toNum(receivingRow.purchase_order_product_id));
            }

            await fetchJson(`${base}/items/${PO_PRODUCTS_COLLECTION}/${productId}`, { method: "DELETE" });
            return ok({ deleted: true, porId: productId });
        }
        ...
        if (action === "open_po") {
            ...
            for (const ln of lines) {
                ...
                const openRow = pors
                    .map(id => porRows.find(r => toNum(r.purchase_order_product_id) === id))
                    .find(r => r && isPreQaRfidAnchor(r) && (rfidsByKey.get(k)?.length || 0) > 0);
                
                const isExtraItem = isExtraPoLine(ln as unknown as Record<string, unknown>);
        ...
        if (action === "tag_and_receive") {
            ...
            if (!matchingLine) {
                return bad("The product is not assigned to this purchase order. Add it as an extra product first.", 409);
            }
        ...
        if (action === "delete_rfid") {
            ...
            const findUrl = `${base}/items/${POR_ITEMS_COLLECTION}?limit=1&filter[rfid_code][_eq]=${encodeURIComponent(rfid)}&fields=receiving_item_id,purchase_order_product_id`;
            const found = await fetchJson<{ data: Array<{ receiving_item_id: number; purchase_order_product_id: number }> }>(findUrl);
            ...
            const anchorDeleted = await cleanupEmptyPreQaRfidAnchor(base, toNum(row.purchase_order_product_id));

            return ok({ deleted: true, rfid, receivingItemId: row.receiving_item_id, anchorDeleted });
        }
