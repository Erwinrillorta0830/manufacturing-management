...
export class JobOrderWorkflowEvidenceError extends Error {
    constructor(
        message: string,
        readonly status = 502,
        readonly code = "WORKFLOW_EVIDENCE_UPLOAD_FAILED"
    ) {
        super(message);
        this.name = "JobOrderWorkflowEvidenceError";
    }
}
...
export async function uploadJobOrderWorkflowEvidence(
    file: File,
    jobOrderNo: string,
    evidenceLabel: "breakdown" | "hold"
): Promise<string> {
    if (!DIRECTUS_URL || !headers.Authorization) {
        throw new JobOrderWorkflowEvidenceError(
    ...
    formData.set("title", `JO ${jobOrderNo} ${evidenceLabel} evidence`.slice(0, 180));
    ...
    if (!response.ok) {
        ...
        throw new JobOrderWorkflowEvidenceError(
            `${evidenceLabel[0].toUpperCase()}${evidenceLabel.slice(1)} evidence upload failed with HTTP ${response.status}: ${message}`,
            forbidden ? 503 : 502,
            forbidden ? "WORKFLOW_EVIDENCE_UPLOAD_FORBIDDEN" : "WORKFLOW_EVIDENCE_UPLOAD_FAILED"
        );
    }
    ...
    if (!uploadedId) {
        throw new JobOrderWorkflowEvidenceError(
            "Workflow evidence upload returned no file identifier.",
            502,
            "WORKFLOW_EVIDENCE_UPLOAD_INVALID_RESPONSE"
        );
    }
    return uploadedId;
}

export async function deleteJobOrderWorkflowEvidence(uploadedId: string): Promise<void> {
    try {
        ...
        if (!response.ok) {
            console.error(`Unable to clean up workflow evidence image ${uploadedId}: HTTP ${response.status}`);
        }
    } catch (error) {
        console.error(`Unable to clean up workflow evidence image ${uploadedId}:`, error);
    }
}

export function jobOrderWorkflowEvidenceImageUrl(uploadedId: string | null | undefined): string | null {
