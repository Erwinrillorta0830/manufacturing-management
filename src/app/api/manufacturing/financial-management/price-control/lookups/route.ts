...
export async function GET(req: NextRequest) {
    try {
        ...
        const productTypesOnly =
            fieldsParam !== null &&
            fieldsParam.size === 1 &&
            fieldsParam.has("product_types");
        ...
        const [categoriesInitial, brandsInitial, unitsInitial, suppliers, productTypesResult] = await Promise.all([
            ...
            fetchSuppliers(),
            fetchDirectus<{ data: { id: number; name: string }[] }>(`${DIRECTUS_URL}/items/product_type?limit=-1&fields=id,name&sort=name`, { headers: directusHeaders() }).catch(() => ({ data: [] })),
        ]);
