...
export async function POST(req: NextRequest) {
    try {
        ...
        const newlyCreatedBatchIds: number[] = [];
        const newlyInsertedReservationIds: number[] = [];
        const consumedReservations: { reservationId: number; originalStatus: string; originalPicked: number }[] = [];
        ...
        try {
            ...
            const hasDispatches = dispatchIds.length > 0;
            if (expectedItemCount > 0 && hasDispatches && !consolidatorId) {
                throw new Error("Failed to resolve consolidator record for this dispatched order. Printing cancelled to prevent incomplete applied quantity updates.");
            }
            ...
            for (const receipt of receipts) {
                ...
                if (!isVoidReplacement && consolidatorId) {
                ...
                for (const item of receipt.items) {
                    let soDetailId: number | null = null;
                    ...
                    if (podRes.ok) {
                        ...
                        if (podData.data && podData.data.length > 0) {
                            soDetailId = Number(podData.data[0].detail_id);
                            if (!isRecycledInvoice && !isVoidReplacement && soDetailId) {
                                ...
                                updatedSalesOrderDetails.push({ detailId: soDetailId, originalQty: currentServed });

                                const updatePodRes = await fetch(`${DIRECTUS_BASE}/items/sales_order_details/${soDetailId}`, {
                                    method: 'PATCH',
                                    headers: directusHeaders(),
                                    body: JSON.stringify({ served_quantity: newServedQty })
                                });
                                if (!updatePodRes.ok) throw new Error(`Failed to update served quantity for detail ${soDetailId}: ${await updatePodRes.text()}`);
                    ...
                    if (targetInvoiceId) {
                        ...
                        // ─────────────────────────────────────────────────────────────
                        // 3b-1: Batch Lot Allocation & Inventory Movement Integration
                        // Bind lots to sales_invoice_batches and set reservation status to Consumed
                        // (Which emits the SALES_ISSUE OUT movement in v_mm_inventory_movements)
                        // ─────────────────────────────────────────────────────────────
                        const effectiveDetailId = existingDetail ? existingDetail.detail_id : processedDetailIds.values().next().value;
                        if (targetInvoiceId) {
                            try {
                                const branchId = typeof order.branch_id === 'object' && order.branch_id !== null 
                                    ? Number(order.branch_id.id) 
                                    : Number(order.branch_id);

                                let remainingToAllocate = Number(item.qty || 0);

                                // 1. Try to find existing reservations for this sales order detail
                                const resvRes = soDetailId ? await fetch(
                                    `${DIRECTUS_BASE}/items/sales_order_reservation?filter[sales_order_detail_id][_eq]=${soDetailId}&filter[status][_in]=Picked,Reserved&fields=reservation_id,sales_order_detail_id,product_id,inventory_lot_id,reserved_quantity,picked_quantity,status&limit=-1`,
                                    { headers: directusHeaders(), cache: 'no-store' }
                                ) : null;

                                if (resvRes?.ok) {
                                    const resvRows = (await resvRes.json()).data || [];
                                    for (const r of resvRows) {
                                        if (remainingToAllocate <= 0) break;
                                        const rQty = Number(r.picked_quantity ?? r.reserved_quantity ?? 0);
                                        if (rQty <= 0) continue;

                                        const allocQty = Math.min(remainingToAllocate, rQty);
                                        const rawInvLot = r.inventory_lot_id;
                                        const invLotId = typeof rawInvLot === 'object' && rawInvLot !== null 
                                            ? Number(rawInvLot.inventory_lot_id || rawInvLot.id) 
                                            : Number(rawInvLot);
                                        ...
                                        if (invLotId) {
                                            const milRes = await fetch(
                                                `${DIRECTUS_BASE}/items/mm_inventory_lots/${invLotId}?fields=lot_id,batch_no`,
                                                { headers: directusHeaders(), cache: 'no-store' }
                                            ).catch(() => null);
                                            if (milRes && milRes.ok) {
                                                const milData = await milRes.json();
                                                lotId = milData.data?.lot_id ? Number(milData.data.lot_id) : null;
                                                batchNo = milData.data?.batch_no || null;
                                            }
                                        }

                                        if (effectiveDetailId && invLotId) {
                                            const sibRes = await fetch(`${DIRECTUS_BASE}/items/sales_invoice_batches`, {
                                                method: 'POST',
                                                headers: directusHeaders(),
                                                body: JSON.stringify({
                                                    invoice_id: targetInvoiceId,
                                                    invoice_detail_id: effectiveDetailId,
                                            ...
                                            if (sibRes.ok) {
                                                ...
                                                if (sibData.data?.id) newlyCreatedBatchIds.push(Number(sibData.data.id));
                                            }
                                        }

                                        consumedReservations.push({
                                            reservationId: Number(r.reservation_id),
                                            originalStatus: r.status,
                                            originalPicked: rQty
                                        });

                                        await fetch(`${DIRECTUS_BASE}/items/sales_order_reservation/${r.reservation_id}`, {
                                            method: 'PATCH',
                                            headers: directusHeaders(),
                                            body: JSON.stringify({
                                                status: 'Consumed',
                                ...
                                // 2. If remainingToAllocate > 0, allocate from available lots
                                if (remainingToAllocate > 0 && branchId) {
                                    const milListRes = await fetch(
                                        `${DIRECTUS_BASE}/items/mm_inventory_lots?filter[product_id][_eq]=${item.product_id}&filter[branch_id][_eq]=${branchId}&sort=expiration_date&limit=-1`,
                                        { headers: directusHeaders(), cache: 'no-store' }
                                    ).catch(() => null);

                                    if (milListRes && milListRes.ok) {
                                        const availLots = (await milListRes.json()).data || [];
                                        for (const l of availLots) {
                                            if (remainingToAllocate <= 0) break;
                                            const lotInvId = Number(l.inventory_lot_id || l.id);
                                            const lotMasterId = l.lot_id ? Number(l.lot_id) : null;
                                            const lotBatchNo = l.batch_no || null;
                                            const allocQty = remainingToAllocate;

                                            const newResvRes = await fetch(`${DIRECTUS_BASE}/items/sales_order_reservation`, {
                                                method: 'POST',
                                                headers: directusHeaders(),
                                                body: JSON.stringify({
                                                    sales_order_detail_id: soDetailId,
                                                    product_id: Number(item.product_id),
                                                    inventory_lot_id: lotInvId,
                                            ...
                                            if (newResvRes.ok) {
                                                const newRJson = await newResvRes.json();
                                                const newRId = newRJson.data?.reservation_id || newRJson.data?.id;
                                                if (newRId) newlyInsertedReservationIds.push(Number(newRId));
                                            }
                                ...
                                console.warn("[generate-invoice] Warning allocating lot batches and movements:", lotErr);
                            }
                        }
                    }
                }
            ...
            if (consolidatorId && results.consolidatorDetailsUpdated !== expectedConsolidatorUpdatesCount) {
            ...
            // 3b. Rollback sales_invoice_batches and restore reservations
            if (newlyCreatedBatchIds.length > 0) {
                for (const bId of newlyCreatedBatchIds) {
                    try {
                        await fetch(`${DIRECTUS_BASE}/items/sales_invoice_batches/${bId}`, {
                            method: 'DELETE',
                            headers: directusHeaders()
                        });
                    } catch (e) {
                        console.error("[Rollback Failed] sales_invoice_batches deletion:", e);
                    }
                }
            }

            if (newlyInsertedReservationIds.length > 0) {
                for (const rId of newlyInsertedReservationIds) {
                    try {
                        await fetch(`${DIRECTUS_BASE}/items/sales_order_reservation/${rId}`, {
                            method: 'DELETE',
                            headers: directusHeaders()
                        });
                    } catch (e) {
                        console.error("[Rollback Failed] sales_order_reservation deletion:", e);
                    }
                }
            }

            if (consumedReservations.length > 0) {
                for (const r of consumedReservations) {
                    try {
                        await fetch(`${DIRECTUS_BASE}/items/sales_order_reservation/${r.reservationId}`, {
                            method: 'PATCH',
                            headers: directusHeaders(),
                            body: JSON.stringify({
                        ...
                        console.error("[Rollback Failed] sales_order_reservation status restoration:", e);
                    }
                }
            }
