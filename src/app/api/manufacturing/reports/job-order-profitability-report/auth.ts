...
const MODULE_PATH = "/mm/reports/job-order-profitability-report";
...
export async function authorizeJobOrderProfitabilityReport(signal?: AbortSignal): Promise<ReportAccessResult> {
    ...
    const payload = await accessResponse.json().catch((error) => {
        if (signal?.aborted) throw error;
        return null;
    });
