...
const MODULE_PATH = "/mm/reports/labor-efficiency-productivity-report";
...
export async function authorizeLaborEfficiencyProductivityReport(): Promise<ReportAccessResult> {
    ...
    if (user.admin === true || user.isAdmin === true || user.role === "ADMIN") return { ok: true };
    ...
    const rows = Array.isArray(payload?.data) ? payload.data as UserModuleAccess[] : [];
