...
async function readLinkedReversals(transferIds: number[], allowMissingMetadata = true): Promise<Map<number, ReversalLinkSummary>> {
...
function attachLinkedReversal(record: LotTransferRecord, link: ReversalLinkSummary | undefined): LotTransferRecord {
...
async function hydrateTransferRecord(record: LotTransferRecord, includeLinkedReversal = true): Promise<LotTransferRecord> {
...
function movementTransactionTypeId(row: RecordValue): number {
...
function reversalInputFromRecord(record: LotTransferRecord, reversalReason: string): LotTransferInput {
    ...
    return {
        ...
        details: details.map((detail) => ({
            lineNo: detail.lineNo,
            productId: detail.productId,
            sourceInventoryLotId: detail.targetInventoryLotId,
